...
if (!globalForRateLimits.__portfolioRateLimits) {
    globalForRateLimits.__portfolioRateLimits = new Map()
}
const rateLimitStore = globalForRateLimits.__portfolioRateLimits
...
export const parseJsonObject = async (
    ...
    if (!contentType.toLowerCase().includes("application/json")) {
        throw new ApiRequestError("Invalid Content-Type, expected application/json", 415)
    }
...
export const requireContentLengthUnderLimit = (request: Request, maxBytes: number) => {
