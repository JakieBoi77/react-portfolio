import { education, educationVisibleThroughSemester } from "../../../_data/portfolio"
...
const getSemesterSortValue = (semesterId: string) => {
    ...
    const seasonOrder = semesterSeasonOrder[season as keyof typeof semesterSeasonOrder] ?? 0

    return Number(year) * 10 + seasonOrder
}

const educationVisibleThroughSortValue = getSemesterSortValue(educationVisibleThroughSemester)
        ...
        levels: educationItem.levels
            .map((level) => ({
                ...
                terms: level.terms.filter(
                    (term) =>
                        getSemesterSortValue(term.semesterId) <= educationVisibleThroughSortValue,
                ),
            }))
...
export const isSpanningCourse = (course: CourseItem) => "spansTerms" in course && course.spansTerms
...
export const getSpanningCoursesCoveringTerm = (level: EducationLevel, targetTermIndex: number) =>
    level.terms.flatMap((term, sourceTermIndex) =>
        term.courses.filter((course) => {
            ...
            const spanCount = getCourseSpanCount(course, sourceTermIndex, level)

            return (
                sourceTermIndex <= targetTermIndex && targetTermIndex < sourceTermIndex + spanCount
            )
        }),
    )
