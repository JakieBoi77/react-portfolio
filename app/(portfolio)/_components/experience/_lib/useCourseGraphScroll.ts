...
export const useCourseGraphScroll = (graphWidth: number) => {
    ...
    const updateGraphScrollbar = useCallback(() => {
        ...
        const thumbWidth = Math.max(
            48,
            (scrollElement.clientWidth / scrollElement.scrollWidth) * trackWidth,
        )
        const maxThumbLeft = Math.max(0, trackWidth - thumbWidth)
        const thumbLeft =
            maxScrollLeft > 0 ? (scrollElement.scrollLeft / maxScrollLeft) * maxThumbLeft : 0
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: graphWidth isn't read directly, but the scrollbar must recompute when the graph's layout width changes (not just on window resize)
    useEffect(() => {
        updateGraphScrollbar()

        const scrollElement = scrollContainerRef.current
        const resizeObserver =
            typeof ResizeObserver !== "undefined" ? new ResizeObserver(updateGraphScrollbar) : null
    ...
    const handleGraphPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
        ...
        const handlePointerMove = (pointerEvent: PointerEvent) => {
            ...
            if (!state.active || state.pointerId !== pointerEvent.pointerId || !scrollElement) {
                return
            }
    ...
    const handleScrollbarPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
        ...
        if (event.button !== 0 || !scrollElement || !trackElement || !thumbElement) {
            return
        }
        ...
        const maxScrollLeft = scrollElement.scrollWidth - scrollElement.clientWidth
        const maxThumbLeft = Math.max(1, trackElement.clientWidth - thumbElement.offsetWidth)

        const handlePointerMove = (pointerEvent: PointerEvent) => {
            const deltaX = pointerEvent.clientX - startX
            scrollElement.scrollLeft = startScrollLeft + (deltaX / maxThumbLeft) * maxScrollLeft
            updateGraphScrollbar()
            pointerEvent.preventDefault()
        }
    ...
    const handleGraphClickCapture = (event: ReactMouseEvent<HTMLDivElement>) => {
