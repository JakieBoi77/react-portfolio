...
const ChoroplethMap = () => {
  ...
  useEffect(() => {
    ...
    svg
      ...
      .selectAll("path")
      .attr("class", "counties")
      .data((topojson.feature(countyData, countyData.objects.counties) as FeatureCollection).features)
      .enter()
      .append("path")
      .attr("class", "county")
      ...
      .on("mouseover", function (event, d) {
        tooltip
          ...
          .style("opacity", 0.8)
          .html(() => {
            let result = educationData.find((obj) => obj.fips === d.id);
            if (result) {
    ...
    svg
      .append("path")
      .datum(
        topojson.mesh(countyData, countyData.objects.states as any, function (a, b) {
          return a !== b;
        })
      )
      .attr("class", "states")
  ...
  return (
    <StyledDiv>
       <div id="app">
        ...
        <p>Source: <a href="https://www.ers.usda.gov/data-products/county-level-data-sets/download-data.aspx">USDA Economoic Research Service</a></p>
      </div>
    </StyledDiv>
  );
}
